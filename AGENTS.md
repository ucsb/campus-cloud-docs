...
* **Run `bundle exec jekyll build`** (if available) to verify the site builds cleanly after structural changes.
* **Site reviews:** When asked to review the site, check what's out-of-date, or suggest improvements, read every page of the site thoroughly. Compare content against the current state of the landing zone git repos and the accessibility guidelines. Present prioritized suggestions to the user — accuracy issues and accessibility violations first, then style and content improvements.
